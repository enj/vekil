...
func publishCarrierParityTurn(t *testing.T, upstreamCallIDs ...string) (*responsesChatReplayStore, responsesChatReplayRoute, []json.RawMessage, responsesChatReplayPublished) {
	...
	for i, id := range upstreamCallIDs {
		...
		calls[i] = responsesChatReplayPublishCall{UpstreamCallID: id, Name: "lookup", VisibleArguments: `{}`, OutputItemIndex: i + 1}
	}
...
func carriedForEveryCall(t *testing.T, route responsesChatReplayRoute, published responsesChatReplayPublished, items []json.RawMessage) map[string]carriedReplay {
	t.Helper()
	signature, err := encodeReasoningCarrier(carriedTurnFromPublished(route, items, published))
	if err != nil {
...
// The carrier changes only WHERE the resolution comes from. The carrier run passes
// no store at all -- what a TTL expiry, eviction or restart leaves behind.
func TestCarriedTurnTranslatesByteIdenticallyToTheStore(t *testing.T) {
	...
	got, want := upstreamInputJSON(t, fromCarrier), upstreamInputJSON(t, fromStore)
	if got != want {
		t.Fatalf("carrier input differs from store input:\ncarrier %s\n  store %s", got, want)
	}
	if !strings.Contains(want, "function_call_output") {
		t.Fatalf("fixture never produced a tool result, so nothing was compared: %s", want)
	}
}
